...
const transferTransactionSchema = z.object({
  ...
  chain: z
    ...
    .optional()
    .describe(
      "The chain where this transaction should execute (e.g., 'Paseo', 'Paseo AssetHub').",
    ),
});
...
const bondExtraPoolTransactionSchema = z.object({
  ...
  extraType: z
    ...
    .optional()
    .describe(
      "Type of bond extra: FreeBalance for additional tokens (requires amount), Rewards for restaking rewards (no amount needed). Use this for NOMINATION POOLS only. Defaults to FreeBalance if amount is provided.",
    ),
  chain: z
    .string()
    .optional()
...
export const batchAgent = tool({
  ...
  execute: async (input) => {
    const { transactions, network } = input as {
      transactions: Transaction[];
      network?: string;
    };

    // Validation: Ensure all transactions target the same network
    const chains = new Set<string>();
    if (network) chains.add(network.toLowerCase());

    for (const tx of transactions) {
      let txChain: string | undefined;
      if (tx.type === "xcm") {
        txChain = tx.src;
      } else if (tx.chain) {
        txChain = tx.chain;
      ...
      if (txChain) {
        const normalizedChain = txChain.toLowerCase();

        // Validation: Staking/Pool operations are NOT allowed on Paseo Relay Chain
        // They must be done on Paseo AssetHub
        const stakingOps = [
          "bond",
          "unbond",
          "bondExtra",
        ...
        // Check if op is staking-related (we accept any case for safety, though schema is lower)
        const isStakingOp = stakingOps.some(
          (op) => op.toLowerCase() === tx.type.toLowerCase(),
        );

        if (normalizedChain === "paseo" && isStakingOp) {
          return {
            message: `Error: Staking and Nomination Pool operations (like '${tx.type}') are NOT supported on the Paseo Relay Chain. You must perform these on 'Paseo AssetHub'. Please switch networks or teleport funds to AssetHub first.`,
          };
        }

        // Check for consistency with existing chains
        for (const existingChain of chains) {
          if (existingChain !== normalizedChain) {
            return {
              message: `Error: Batch transaction cannot mix actions for different networks ('${existingChain}' vs '${titleCase(normalizedChain)}'). Batching is only allowed for a single network. Please execute these actions separately.`,
            };
          }
        }
    ...
    for (const tx of transactions) {
      ...
      if (txChain) {
        ...
        for (const existingChain of chains) {
          if (existingChain !== normalizedChain) {
            return {
              message: `Error: BatchAll transaction cannot mix actions for different networks ('${existingChain}' vs '${titleCase(normalizedChain)}'). Atomic batching is only allowed for a single network. Please execute these actions separately.`,
            };
          }
        }
        chains.add(normalizedChain);
